...
SimulateJointLongData <- function(n.sample = 100,
                                  ...
                                  zeta.long,
                                  # partition-specific effect of longitudinal marker
                                  eta.long,
                                  # time-independent effect of baseline covariates
                                  beta.long,
                                  ...
                                  cens.poten.rate = 0)
{
  ...
  if (length(zeta.long) != length(times) + 1) {stop("zeta.long should have the same length as times")}
  ...
  # baseline: all patients are at risk
  N.T <- array(0,
               dim = c(n.sample,
                      length(times_incl_zero)))
  ...
  for(k in seq(2, K+1)) {
    ...
    Y.traj[, k] <- as.matrix(rnorm(n = n.sample,
                                   mean = zeta.long[k] + X %*% beta.long +
                                     eta.long[k - 1] * Y.traj.stand.k_1,
                                   sd = sd.long.trajectory))

    # standardize marker
    Y.traj.stand.k <- (Y.traj[, k] - mean(Y.traj[, k])) / sqrt(var(Y.traj[, k]))
