  ...
  for (i in seq(1, n)) {
    ...
    for (t in seq_along(tij[-1]) + 1) {
      ...
      if (it.risk.T == 1) {
        ...
        # not at risk for death in future
        if (Y.T[i, t] == 1) {
          risk.T[i, t] <- 0
          risk.C[i, t] <- 0
        }

        # if not dead, at risk for censoring
...
# Estimation --------------------------------------------------------------
dataDARMAR$LMM <- predict(LMM.DARMAR, newdata = dataDARMAR)
dataDNARMAR$LMM <- predict(LMM.DNARMAR, newdata = dataDNARMAR)

dataDARMAR$IEE <- predict(IEE.DARMAR, newdata = dataDARMAR)
dataDNARMAR$IEE <- predict(IEE.DNARMAR, newdata = dataDNARMAR)

dataDARMAR$GEE.CT <- predict(GEE.CT.DARMAR, newdata = dataDARMAR)
dataDNARMAR$GEE.CT <- predict(GEE.CT.DNARMAR, newdata = dataDNARMAR)

dataDARMAR$GEE.C <- predict(GEE.C.DARMAR, newdata = dataDARMAR)
dataDNARMAR$GEE.C <- predict(GEE.C.DNARMAR, newdata = dataDNARMAR)

dataDNARMAR$JM <- predict(JM.DNARMAR, newdata = dataDNARMAR)

patients.DARMAR <- dataDARMAR %>%
  group_by(time, X) %>%
